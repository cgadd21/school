-- Updated March 8, 2021 to include ON Delete  ON Update
-- Updated by Professor Jim Habermas
-- Updated February 28, 2020
-- ENGINE=InnoDB DEFAULT CHARSET=utf8
-- variable sql_mode NO_AUTO_CREATE_USER
...
CREATE TABLE `phones` (
  ...
  CONSTRAINT `phones_ibfk_1` FOREIGN KEY (`PassengerId`) 
...
CREATE TABLE `trip_people` (
  ...
  CONSTRAINT `trip_people_ibfk_3` FOREIGN KEY (`PassengerID`) 
        REFERENCES `passenger` (`PassengerID`)
           ON DELETE CASCADE
             ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
--
/*!50003 DROP PROCEDURE IF EXISTS `getEquipInfo` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getEquipInfo`(IN eName varchar(50), OUT eId INT(11), OUT eDesc VARCHAR(100), OUT eCapacity int(11), OUT avgCap  Double(8,2) )
BEGIN
    SELECT equipId, EquipmentDescription, EquipmentCapacity into eId, eDesc, eCapacity
    from Equipment where equipmentName like  Concat("%",eName,"%");
    
    select avg(equipmentcapacity) into avgCap from equipment;
END ;;
DELIMITER ;
...
/*!50003 DROP PROCEDURE IF EXISTS `getPassengerInfo` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `getPassengerInfo`(in fn varchar(50), in ln varchar(50),
                    out passid int(11), out str varchar(50), out zipcode varchar(5) )
BEGIN
    select passengerid, street, zip into  passid, str, zipcode 
            from passenger where concat(fname," ",lname) = concat(fn,' ',ln);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2018-11-15 15:11:47
